...
class ThumbnailsTest5 extends JFrame{
 	private static final long serialVersionUID = 523285448953446819L;
 	private BufferedImage originalImage = null;
	private BufferedImage thumbnail = null;
    public ThumbnailsTest5(){
    	super("이미지 그리기");
        try {
        	// 200 *  200 섬네일 이미지를 스케일 조정해서 BufferedImage로 만듬
        	originalImage = ImageIO.read(new File("java.png"));
         	thumbnail = Thumbnails.of(originalImage).scale(0.5).asBufferedImage();
        } catch (IOException e) {
			e.printStackTrace();
		}
